  ...
  request(URL, async (err: any, response: any, html: any) => {
    if (!err && response.statusCode === 200) {
      ...
      parentContainer.each(async (i, element) => {
        ...
        const video_dt = await getVideoDetails(`https://xvideos.com${videoLink}`);
        ...
        const VideoItem: IVideo = {
          ...
          video_image: $(element).prev().find('.thumb').find('img').attr('data-src') as string,
...
export const getVideoDetails = (URL: string): Promise<{ url: string; tags: string }> => {
  return new Promise((resolve, reject) => {
    request(URL, async (err: Error, response: any, html: any) => {
      if (!err && response.statusCode === 200) {
        ...
        // @ts-ignore
        const getVideoUrlHigh = getVideoPlayerScript.match(/setVideoUrlHigh+\('+([a-zA-Z0-9]).+(?='\))/g);
        // @ts-ignore
        const getVideoUrlLow = getVideoPlayerScript.match(/setVideoUrlLow+\('+([a-zA-Z0-9]).+(?='\))/g);

        if (getVideoUrlHigh) {
          resolve({ url: getVideoUrlHigh[0].split("('")[1], tags: videoTagList.slice(0, -1) });
        } else if (getVideoUrlLow) {
          resolve({ url: getVideoUrlLow[0].split("('")[1], tags: videoTagList.slice(0, -1) });
        } else {
